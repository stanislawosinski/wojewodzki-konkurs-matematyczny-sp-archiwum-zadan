...
Jeden plik na arkusz, tak samo nazwany jak `browser/data/<plik>.json`. Zawartość: tylko zadania,
...
const todo=[];
for(const f of readdirSync("browser/data/").filter(f=>f.endsWith(".json"))){
 if(existsSync("dev/solutions/"+f))continue;
 const n=JSON.parse(readFileSync("browser/data/"+f,"utf8")).questions.filter(x=>{const a=x.answer||{};
...
Pozostałe pola args: `dataDir` = `browser/data`, `figDir` = `browser/figures`,
`outDir` = `dev/solutions`, `model` = `sonnet` — wszystkie jako ścieżki bezwzględne. Partie po ~80
...
nic nie nadpisało sidecarów kampanii. Scalenie do `dev/solutions/<arkusz>.json` jest osobnym,
