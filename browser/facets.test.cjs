...
const yes = () => true;
const sel = o => { const s = { topic: new Set(), woj: new Set() }; for (const k in o) s[k] = new Set(o[k]); return s; };
const sorted = a => a.slice().sort();

// no selection -> everything
assert.deepStrictEqual(sorted(matchedHashes(index, sel({}), yes, universe)), ['h1', 'h2', 'h3', 'h4']);
// single value -> that value's set
assert.deepStrictEqual(sorted(matchedHashes(index, sel({ topic: ['potęgi'] }), yes, universe)), ['h1', 'h2']);
// OR within a facet -> union
assert.deepStrictEqual(sorted(matchedHashes(index, sel({ topic: ['potęgi', 'pierwiastki'] }), yes, universe)), ['h1', 'h2', 'h3', 'h4']);
// AND across facets -> intersection
assert.deepStrictEqual(sorted(matchedHashes(index, sel({ topic: ['potęgi'], woj: ['slaskie'] }), yes, universe)), ['h1']);

// drill-down: a facet ignores its OWN selection (siblings show what they'd add)
assert.deepStrictEqual(facetCounts(index, sel({ topic: ['potęgi'] }), yes, 'topic', universe), { 'potęgi': 2, 'pierwiastki': 2 });
// drill-down: OTHER facets reflect the selection (breakdown of the potęgi set)
assert.deepStrictEqual(facetCounts(index, sel({ topic: ['potęgi'] }), yes, 'woj', universe), { 'slaskie': 1, 'malopolskie': 1 });
